...
- (void) MR_addObject:(NSManagedObject *)relatedObject forRelationship:(NSRelationshipDescription *)relationshipInfo
{
    ...
    NSAssert2([relatedObject entity] == [relationshipInfo destinationEntity] || [relatedObject entity].superentity == [relationshipInfo destinationEntity], @"related object entity %@ not same as destination entity %@", [relatedObject entity], [relationshipInfo destinationEntity]);
...
- (void) MR_importValuesForKeysWithDictionary:(id)objectData
{
    ...
    if ([self respondsToSelector:@selector(willImport:)])
    {
        id modifedObjectData = [self performSelector:@selector(willImport:) withObject:objectData];
        if (modifedObjectData && strcmp([[self methodSignatureForSelector:@selector(willImport:)] methodReturnType], @encode(void)) != 0)
        {
            objectData = modifedObjectData;
        }
    }
